...
bool RIFF::AIFF::File::isSupported(IOStream *stream)
{
  ...
  /*return id.startsWith("FORM") && (id.containsAt("AIFF", 8) || id.containsAt("AIFC", 8));/*/  // dro change
  return (!id.isEmpty() && id.startsWith("FORM") && (id.containsAt("AIFF", 8) || id.containsAt("AIFC", 8)));/**/
}
